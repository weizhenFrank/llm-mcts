...
Allowed actions: walk to <object or room>, grab <object>, open <container>, close <container>, put <object> on <surface>, put <object> in <container>.
Rooms in the house: bedroom, bathroom, living room, kitchen
You can only grab one object at all time. If you have grabbed an object, you can only put it somewhere to grab another object.
...
You need to strictly follow the format in the following examples:\n"""
    ...
    def ground_actions_softmax(self, task, instruction, observation, valid_action_embedding, valid_actions):
        ...
        for sample in samples:
            ...
            softmax = np.exp(100 * cos_sim) / np.sum(100 * np.exp(cos_sim), axis=0)
